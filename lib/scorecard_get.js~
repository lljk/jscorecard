...
//var failed = [];
//console.log('in get:', failed_fetches);/////////
...
function get_game_header(game_url, callback) {
	...
	get_json(url, function(json) {
			...
			function get_boxscore(callback) {//game_url, callback) {
...
function get_bases(atbat, index, atbats, callback) {
	var bases = [], pitch = null;
	var next_atbat = atbats[index + 1];
	if (next_atbat){
		pitch = next_atbat.pitch[0]
	}
	else{
		pitch = atbat.pitch[atbat.pitch.length - 1]
	}
	if (pitch) {
...
function xget_bases(atbat, index, atbats, callback) {
	var bases = [], onbase = null;
	if (atbat.runner) {
