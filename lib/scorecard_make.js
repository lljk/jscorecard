...
function make_card(home_or_away) {
	...
	get_game_header(url, function(away_team, home_team, boxscore, park) {
			...
			get_innings(url, function(away, home) {
					...
					get_players(url, away, home, function(away_batters, home_batters, away_pitchers,
						home_pitchers){
...
function make_atbat_cell(cell, atbat, inning, description, multi_atbat) {
	...
	var diamond = $(cell.children()[1]);
	diamond.append('<canvas class="basepath" id="basepath' + atbat.num + '"></canvas>');
	var basepath = $('#basepath' + atbat.num)
	...
	get_event(atbat, function(note, hit){
			var note_box = $('<div class="note"></div>')
			if ((atbat.des).match('error')) {
				note_box.append("<div class='error'>E</div>");
			};
			if (note == 'Kc') {
				note = 'K';
				note_box.prepend('<div id="called_strikeout">' + note + '</div>');
				cell.append(note_box);
			}
			else {
				note_box.prepend(note);
				cell.append(note_box);
			};
			...
			if (hit){
				switch (hit){
				case 'single':
					basepath.css('border-color', 'transparent transparent ' + basecolor + ' transparent'); break;
				case 'double':
					basepath.css('border-color', 'transparent ' + basecolor + ' ' + basecolor + ' transparent'); break;
				case 'triple':
					basepath.css('border-color', basecolor + ' ' + basecolor + ' ' + basecolor + ' transparent'); break;
				case 'home_run':
					basepath.css('border-color', basecolor); break
				};
			};
	});
	
	get_bases(atbat, index, inning, function(bases) {
			if (bases) {
				var canvas = basepath[0].getContext('2d');
				var base_size = basepath[0].width * 0.125;
				var w = basepath[0].width, h = basepath[0].height;
				canvas.fillStyle = basecolor //'#0B610B';
				$(bases).each(function(i, base){
					switch (parseInt(base)){
					case 1:
						canvas.fillRect(w - base_size * 2, h - base_size, base_size * 2, base_size); break;
					case 2:
						canvas.fillRect(w - base_size * 2, 0, base_size * 2, base_size); break;
					case 3:
						canvas.fillRect(0, 0, base_size * 2, base_size); break;
					};
				});
			};
	});
	...
	if (multi_atbat) {
		cell.append('<div class="multi_atbat">+</div>');
	};
	...
	cell.click(function(event){
			var pitches;
			get_pitches(atbat, function(ps) {
					pitches = ps
			});

			var popup_body = '<div class="atbat_wrapper"><div class="popup_text">' + description + '</div><div class="pitches_wrapper"><div class="pitches_frame"><div class="strike_zone"><div class="pitch_spacer"></div></div></div><div class="pitch_spacer"></div></div>'
				
			make_popup('At-Bat&nbsp' + atbat.num, popup_body, event);
						
			$(pitches).each(function(i, pitch){
					$('.pitches_frame').append('<div class="pitch" id="pitch' + i + '"><p>' + (i + 1) + '</p></div>');
					var pitch_x = pitch[0], pitch_y = pitch[1], des = pitch[2];
					var width = parseInt($('.pitches_frame').css('width')), height = parseInt($('.pitches_frame').css('width'));
					var x = ((pitch_x * width) / 100) - (width * 0.55);
					var y = ((pitch_y * height) / 75) - (height * 1.35);
					$('#pitch' + i).css('left', x);
					$('#pitch' + i).css('top', y);
					if (des.match(/Ball/)) {$('#pitch' + i).css('background-color', '#00AA44');}
					else if (des.match(/Strike/)) {$('#pitch' + i).css('background-color', '#880000');}
					else if (des.match(/Foul/)) {$('#pitch' + i).css('background-color', '#555555');}
					else if (des.match(/play/)) {$('#pitch' + i).css('background-color', '#000088');};
			});
	});
