...
function make_input_form(){
	...
	$('#input_form').append('<div id="select_date"><h2>Select Date: <input type="text" id="datepicker"/></h2></div>');
	$('#datepicker').datepicker({dateFormat: 'D, M d, yy'});
	
	$('#datepicker').change(function(){
			if ($('#search_btn')) { $('#search_btn').remove() };
			if ($('.game_listing')) { $('.game_listing').remove() };
			$('#input_form').append('<button id="search_btn"><big>Search for 	Games</big></button>');
...
function make_games_list(games, callback) {
	...
	$(games).each(function(i, game) {
			...
			$('#input_form').append('<p class="game_listing" id="game_listing_' + game.gameday + '"><big>' + entry + '</big></p>');
...
function make_batting_table(home_or_away) {
	...
	$(batters).each(function(i, batter){
			...
			get_batter_stats(batter, function(stats){
					...
					$('#player_box_' + batter.id).click(function(e){
							make_batter_popup(stats[0], stats[1], function(table){
									make_popup(batter.first_name + '&nbsp' + batter.last_name, table, e.pageY);
							});
					});
			});
...
function make_atbat_cell(cell, atbat, inning, description, multi_atbat) {
	...
	cell.click(function(e){
			make_popup('At-Bat&nbsp' + atbat.num, description, e.pageY);
	});
	cell.hover(function(){
...
function make_pitching_table(home_or_away) {
	...
	$('body').append('<br><br><hr><h1>Pitching</h1>');
	...
	$(pitchers).each(function(i, pitcher) {
			if (pitcher.id) {
				...
				$('#pitcher_box_' + pitcher.id).click(function(e){
						make_pitcher_popup(pitcher, function(table){
								make_popup(pitcher.name_display_first_last, $(table), e.pageY)
						});
				});
			};
...
function make_footer(){
	$('body').append('<div class="help_button"><p>?</p></div>');
	$('.help_button').click(function(e){
			make_help_box(function(help){
					make_popup('Scorecard Generator', help, e.pageY - 250);
					console.log(help.parents()[1]);
	...
	function make_help_box(callback) {
		...
		about_help.append('<a rel="license" href="http://creativecommons.org/licenses/by-nc/3.0/deed.en_US"></a><br /><span xmlns:dct="http://purl.org/dc/terms/" property="dct:title">jScorecard</span> by <span xmlns:cc="http://creativecommons.org/ns#" property="cc:attributionName">J. Kaiden</span> is licensed under a <a rel="license" href="http://creativecommons.org/licenses/by-nc/3.0/deed.en_US">Creative Commons Attribution-NonCommercial 3.0 Unported License</a>.<br />Based on a work at <a xmlns:dct="http://purl.org/dc/terms/" href="https://github.com/lljk/jscorecard" rel="dct:source">https://github.com/lljk/jscorecard</a>.<br><br><img alt="Creative Commons License" style="border-width:0" src="http://i.creativecommons.org/l/by-nc/3.0/88x31.png" />');
		...
		var close_btn = $('<div id="about_close_button">ok</div>');
...
function make_popup(title, body, top){
	if ($('.popup_box')) {$('.popup_box').remove()};
	var d = new Date(); id = d.getTime();
	$('body').append('<div class="popup_box" id="popup_box' + id + '"></div>');
	$('#popup_box' + id).append('<div class="popup_title" id="popup_title' + id + '">' + title + '</div>')
	$('#popup_box' + id).append('<div class="popup_body" id="popup_body' + id + '"></div>');
	$('#popup_body' + id).html(body);

	$('#popup_box' + id).css("top", top + 20);
	$('#popup_box' + id).hide();
	$('#popup_box' + id).fadeIn("slow");
	$('#popup_box' + id).click(function(){
			$('#popup_box' + id).remove();
	});
