...
function get_games(){
	...
	game_url = 'https://gd2.mlb.com/components/game/mlb/'+ 'year_' + d[0] + '/month_' + d[1] + '/day_' + d[2] + '/';
...
function get_players(game_url, away_innings, home_innings, callback) {
	...
	$(boxscore.batting).each(function(i, team) {
			var batters = team.batter
			switch (team.team_flag) {
				case 'away':
					$(batters).each(function(i, batter) {
							if (batter.bo) {away_bat_ids.push(batter.id)};
					}); break;
				case 'home':
					$(batters).each(function(i, batter) {
							if (batter.bo) {home_bat_ids.push(batter.id)};
					}); break;	
				};
		});
...
function get_pitches(atbat, callback) {
	var pitches = [];
	if (atbat.pitch.length) {
		$(atbat.pitch).each(function(i, pitch) {pitches.splice(i, 1, [parseInt(pitch.x), parseInt(pitch.y), pitch.des])});
	}
	else {pitches.push([parseInt(atbat.pitch.x), parseInt(atbat.pitch.y), pitch.des])};
